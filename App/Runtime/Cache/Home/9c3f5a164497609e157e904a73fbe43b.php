...
<link rel="stylesheet" type="text/css" href="__PUBLIC__/css/mycoupon.css" />
	<ul class="expire_filter_box">
		<?php if($_GET['status'] == ''): ?><li><a href="<?php echo U('Account/mycoupon');?>" class="hover">全部</a></li>
		<?php else: ?>
			<li><a href="<?php echo U('Account/mycoupon');?>">全部</a></li><?php endif; ?>
		<?php if($_GET['status'] == '1'): ?><li><a href="<?php echo U('Account/mycoupon',array('status' => 1),'','');?>" class="hover">未到期</a></li>
		<?php else: ?>
			<li><a href="<?php echo U('Account/mycoupon',array('status' => 1),'','');?>">未到期</a></li><?php endif; ?>
		<?php if($_GET['status'] == '2'): ?><li><a  href="<?php echo U('Account/mycoupon',array('status' => 2),'','');?>" class="hover">已到期</a></li>
		<?php else: ?>
			<li><a  href="<?php echo U('Account/mycoupon',array('status' => 2),'','');?>">已到期</a></li><?php endif; ?>
		
	</ul>
	<div class="myconpon_items">
		<table>
			<tbody>
				<tr>
					<th class="one">优惠劵名称</th>
					<th class="two">下载时间</th>
					<th class="three">状态</th>
					<th class="four">操作</th>
				</tr>
                <?php if(is_array($coupons)): foreach($coupons as $key=>$coupon): ?><tr>
					<td ><a href="<?php echo U('Coupon/detail',"","","");?>/<?php echo ($coupon["coupon_id"]); ?>" class="couponname"><?php echo ($coupon["name"]); ?></a></td>
					<td class="center"><?php echo ($coupon["createtime"]); ?></td>
                    <?php if(($coupon["status"] == 1) ): ?><td class="center green">未到期</td>
                    <?php elseif($coupon["status"] == 0): ?>
                    <td class="center ">已使用</td>
                    <?php else: ?>
                    <td class="center ">已到期</td><?php endif; ?>
					<td>
						<a class="operation01">发送到手机</a><a href="<?php echo U("Home/Account/mytocomment");?>" class="operation02">我要评价</a><a href="<?php echo U("Home/Account/mycommented");?>" class="operation03">查看我的评价</a>

					</td>
				</tr><?php endforeach; endif; ?>
			</tbody>
		</table>
		<div id="send_cellphone_hidden_box">
				...
				<form method="post" action="<?php echo U("Home/Account/handleSendToCellphone");?>">
					<ul>
						<li>
							...
							<p class="hidden_error_tips" id="send_phone_error_tip">输入的手机号不能为空</p>
						</li>
						<li>
							<p class="label">验证码</p>
							<input type="text" name="send_cellphone_vcode" id="send_cellphone_vcode" class="shorter" />
							<a href="" id="send_cellphone_vcode_not_clear">看不清</a>
							<img src="<?php echo U("Home/Login/verifyImg");?>" id="send_cellphone_vcode_img">
							<p class="hidden_error_tips" id="send_vcode_error_tip">输入的验证码不能为空</p>
						</li>
					</ul>
					<input type="submit" value="绑定" name="send_cellphone_submit_btn" id="send_cellphone_submit_btn">
				</form>
			</div>
		</div>
	</div>
