...
<link rel="stylesheet" type="text/css" href="__PUBLIC__/css/myfavorite.css" />
	<ul class="expire_filter_box">
		<li><a href="" class="hover">全部</a></li>
		<li><a href="/index.php/account/myfavorite?status=1">未到期</a></li>
		<li><a href="/index.php/account/myfavorite?status=2">已到期</a></li>
	</ul>
	<div class="myconpon_items">
		<table>
			<tbody>
				<tr>
					<th class="one">项目名称</th>
					<!-- <!TODO--th class="two">收藏人气</th--> 
					<th class="three">状态</th>
					<th class="four">操作</th>
				</tr>
                <?php if(is_array($coupons)): foreach($coupons as $key=>$coupon): ?><tr>
					<td ><?php echo ($coupon["title"]); ?></td>
					<!-- <!TODO--  td class="center">400</td--> 
                    <?php if($coupon["end"] == 0): ?><td class="center green">未到期</td>
                    <?php else: ?>
                    <td class="center">已到期</td><?php endif; ?>
					<td>
						<a class="operation01">发送到手机</a><a href="__ACTION__/couponid/<?php echo ($coupon["coupon_id"]); ?>" class="operation02_del">删除</a>
					</td>
